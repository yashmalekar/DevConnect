...
const Following = () => {
  ...
  const userFollowing = useSelector((state) => state.auth.data.following);
  ...
  const setFollowingData = async()=>{
    const followingData = await fetch('http://localhost:5000/get-users').then(res=>res.json());
    const data = followingData.filter((user:any)=>Array.isArray(userFollowing) && userFollowing.includes(user.uid));
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-slate-400">People you follow ({following.length})</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {following.length > 0 ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-slate-400 mb-4">Discover and follow developers in your field</p>
              <Button 
